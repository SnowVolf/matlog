...
public class ScrubberUtils {
    ...
    private static final Pattern EMAIL_PATTERN = PatternsCompat.EMAIL_ADDRESS;
    ...
    private static final Pattern WEB_URL_PATTERN = Pattern.compile("\\b(https?|ftp|file)://[-a-zA-Z0-9+&@#/%?=_|!:,.;]*[-a-zA-Z0-9+&@#/%?=|]");
    private static final Pattern IP_ADDRESS_PATTERN = PatternsCompat.IP_ADDRESS;
    ...
    private static final Pattern USER_INFO_PATTERN = Pattern.compile("(?:[a-zA-Z0-9$\\-_.+!*'()" + ",;?&=]|(?:%[a-fA-F0-9]{2})){1,64}(?::(?:[a-zA-Z0-9$\\-_" + ".+!*'(),;?&=]|(?:%[a-fA-F0-9]{2})){1,25})?@");
